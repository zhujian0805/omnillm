...
describe("Anthropic to OpenAI translation logic", () => {
  test("should translate Anthropic haiku aliases to available Alibaba models", () => {
    const originalProvider = state.currentProvider
    const originalModels = state.models

    state.currentProvider = { id: "alibaba" } as typeof state.currentProvider
    state.models = {
      object: "list",
      data: [
        {
    ...
    try {
      ...
      expect(openAIPayload.model).toBe("qwen3-coder-flash")
    } finally {
      state.currentProvider = originalProvider
      state.models = originalModels
    }
  })

  test("should translate Anthropic sonnet aliases to available Alibaba models", () => {
    const originalProvider = state.currentProvider
    const originalModels = state.models
    ...
    try {
      ...
      const openAIPayload = translateToOpenAI(anthropicPayload)
      expect(openAIPayload.model).toBe("qwen3-coder-plus-2025-09-23")
    } finally {
      state.currentProvider = originalProvider
      state.models = originalModels
    }
  ...
  test("should normalize shorthand Anthropic tool schemas to valid object schemas", () => {
    const anthropicPayload: AnthropicMessagesPayload = {
      model: "gpt-4o",
      messages: [{ role: "user", content: "Hello!" }],
      max_tokens: 128,

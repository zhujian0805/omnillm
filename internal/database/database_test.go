...
func TestChatStoreCRUD(t *testing.T) {
	store := NewChatStore()
	if err := store.CreateSession("session-1", "Title", "model-1", "openai", "omnicode"); err != nil {
...
func TestChatStoreOrdersMessagesDeterministically(t *testing.T) {
	store := NewChatStore()
	if err := store.CreateSession("session-order", "Title", "model-1", "openai", "omnicode"); err != nil {
		t.Fatalf("create session failed: %v", err)
	}
...
func TestChatStoreOrdersSessionsDeterministically(t *testing.T) {
	...
	for _, sessionID := range []string{"session-b", "session-a", "session-c"} {
		if err := store.CreateSession(sessionID, sessionID, "model-1", "openai", "omnicode"); err != nil {

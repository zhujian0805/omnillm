// Package antigravity — Antigravity SSE stream parser.
package antigravity
...
func ParseAntigravitySSE(body io.ReadCloser, eventCh chan cif.CIFStreamEvent) {
	...
	for scanner.Scan() {
		...
		if !streamStartSent {
			eventCh <- cif.CIFStreamStart{Type: "stream_start", ID: shared.RandomID(), Model: "antigravity"}
...
// StopReason converts an Antigravity finishReason to a CIF stop reason.

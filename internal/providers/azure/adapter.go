...
func (p *Provider) GetID() string         { return string(types.ProviderAzureOpenAI) }
...
func (p *Provider) SetupAuth(options *types.AuthOptions) error {
	token, endpoint, cfg, err := SetupAuth(p.instanceID, options)
	if err != nil {
		return err
	}
	p.token = token
	if endpoint != "" {
		p.baseURL = endpoint
		if name := deriveName(endpoint); name != "" {
			p.name = name
		}
	}
	if cfg != nil {
...
func (p *Provider) GetModels() (*types.ModelsResponse, error) {
	p.loadConfigFromDB()
	return GetModels(p.instanceID, p.config), nil
}
...
func (p *Provider) applyConfig(config map[string]interface{}) {
	...
	if endpoint, ok := firstString(config, "endpoint"); ok {
		...
		if name := deriveName(endpoint); name != "" {
			if p.name == "" || p.name == p.instanceID || p.name == "Azure OpenAI" {
				p.name = name
			}
		}
	}
...
func (a *Adapter) RemapModel(model string) string {
	return RemapModel(a.provider.config, strings.TrimSpace(model))
}

func (a *Adapter) azureResponsesURL() (string, error) {
	return ResponsesURL(a.provider.baseURL)
}
...
func (a *Adapter) Execute(ctx context.Context, request *cif.CanonicalRequest) (*cif.CanonicalResponse, error) {
	...
	return ExecuteResponses(ctx, url, a.provider.token, request, model)
}
...
func (a *Adapter) ExecuteStream(ctx context.Context, request *cif.CanonicalRequest) (<-chan cif.CIFStreamEvent, error) {
	...
	model := a.RemapModel(request.Model)
	return StreamResponses(ctx, url, a.provider.token, request, model)
}
...
func deriveName(endpoint string) string {
	if endpoint == "" {
		return ""
	}
